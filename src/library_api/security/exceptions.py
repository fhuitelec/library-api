...
async def jwt_exception_handler(_: Request, exc: PyJWTError) -> Response:
    ...
    return JSONResponse(AuthenticationResponse.jwt_error(exc).model_dump(), status_code=HTTPStatus.UNAUTHORIZED)
...
async def unauthorized_exception_handler(_: Request, exc: HTTPException) -> Response:
    ...
    if exc.status_code == HTTPStatus.UNAUTHORIZED:
        return JSONResponse(
            AuthenticationResponse.unauthenticated(exc).model_dump(), status_code=HTTPStatus.UNAUTHORIZED
        )
